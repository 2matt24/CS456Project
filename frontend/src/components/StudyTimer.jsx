...
function StudyTimer() {
    ...
    const handleTimerComplete = async () => {
        ...
        // Calculate duration
        const duration = sessionType === 'study' ? STUDY_TIME : BREAK_TIME;
        const durationMinutes = Math.floor(duration / 60);

        // Save to backend
        try {
            await studySessionsAPI.create(courseId, sessionType, durationMinutes);
            alert(sessionType === 'study' ? 'Study session complete! Take a break.' : 'Break over! Ready to study?');
        } catch (error) {
            console.error('Failed to save session:', error);
            alert(sessionType === 'study' ? 'Study session complete!' : 'Break over!');
        }
    };
